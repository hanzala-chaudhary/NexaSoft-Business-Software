...
// baqi saara code aur imports neechay wese hi rahenge...import React, { useMemo, useState } from "react";
import {
  Search,
  Boxes,
  PackageCheck,
...
const brands = ["Logitech", "HP", "Samsung", "Anker", "Philips", "Generic"];
const categories = ["Electronics", "Stationery", "Home", "Accessories", "Furniture"];
...
function makeDummyInventory(count: number): InventoryItem[] {
  const stockLevels = [0, 2, 5, 9, 14, 22, 48, 90, 150, 310];
  return Array.from({ length: count }, (_, i) => {
    const currentStock = stockLevels[i % stockLevels.length];
    const reserved = currentStock === 0 ? 0 : Math.min(currentStock, (i * 3) % 6);
    return {
      id: `INV-${3000 + i}`,
      product: productNames[i % productNames.length],
      sku: `SKU-${2000 + i}`,
      barcode: `8${(900000000000 + i * 137).toString().slice(0, 12)}`,
...
const allItems: InventoryItem[] = makeDummyInventory(42);
...
export default function Inventory() {
  ...
  const summary = useMemo(() => {
    ...
    const totalReserved = allItems.reduce((sum, i) => sum + i.reserved, 0);
    const lowStockCount = allItems.filter(
      (i) => getStatus(i.currentStock) === "Low Stock"
    ).length;
    const outOfStockCount = allItems.filter(
      (i) => getStatus(i.currentStock) === "Out of Stock"
    ).length;
                ...
                {filtered.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{item.currentStock}</td>
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{item.reserved}</td>
                      <td className="px-4 py-3 font-medium text-slate-800 whitespace-nowrap">
                        {available}
                      </td>
